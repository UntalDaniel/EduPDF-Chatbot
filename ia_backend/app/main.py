...
from app.services.pdf_processor import (
    process_pdf_from_storage_url, 
    process_uploaded_pdf, 
    list_user_pdfs_from_db,
    delete_pdf_from_firestore_and_storage 
)
# CORRECCIÓN AQUÍ: Cambiar get_vector_store_for_pdf a get_vector_store_for_pdf_retrieval
# También, delete_pdf_vector_store se llama delete_pdf_vector_store_namespace en tu rag_chain.py
from app.services.rag_chain import get_rag_response, delete_pdf_vector_store_namespace, get_vector_store_for_pdf_retrieval 
...
async def upload_and_process_pdf_endpoint(
    ...
    try:
        ...
        return PDFProcessResponse(message=result.get("message", "Error procesando PDF subido."), pdf_id=result.get("pdf_id", pdf_id), filename=file.filename)
...
async def delete_pdf_endpoint(pdf_id: str, user_id: str = Query(..., description="ID del usuario propietario del PDF para verificación.")): 
    ...
    try:
        pinecone_deleted = await delete_pdf_vector_store_namespace(pdf_id) 
        if pinecone_deleted:
    ...
    except PermissionError as pe: # Capturar el error de permiso de delete_pdf_from_firestore_and_storage
...
async def query_pdf_endpoint(request: QueryRequest): 
    ...
    try:
        # Asumiendo que get_rag_response ahora toma 'chat_history_from_frontend'
        chat_response_obj = await get_rag_response(
            pdf_id=request.pdf_id,
            query_text=request.query,
            chat_history_from_frontend=request.chat_history, 
            user_id=request.user_id,
            # language y model_id no están en QueryRequest, get_rag_response usará defaults.
        )
...
async def chat_rag_endpoint(
    pdf_id: str,
    request_body: ChatRequestBody # Usar el schema ChatRequestBody
):
...
async def api_generate_exam_questions_endpoint( 
    ...
    try:
        ...
        if not generated_exam_data.questions and (request.question_config.vf_questions > 0 or request.question_config.mc_questions > 0):
            ...
            # Devolver el objeto tal cual, el frontend puede interpretar la lista vacía o el error si se añadió.
            # Si el servicio ya puso un error en generated_exam_data.error, se manejará arriba.
            # Si no hay error y no hay preguntas, es que el LLM no las generó.
...
if __name__ == "__main__":
    ...
    reload_flag = getattr(settings, "RELOAD", True)
    log_level_str = getattr(settings, "LOG_LEVEL", "info").lower()

    logger.info(f"Starting Uvicorn server on host {host} and port {port}, reload: {reload_flag}, log_level: {log_level_str}")
    uvicorn.run(
        "main:app",
        host=host,
        port=port, 
